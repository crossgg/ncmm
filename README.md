...
5. **🎖️ 音乐人任务与黑胶会员进阶任务 (`ncmm musician-sign` / `ncmm musician-vip` / `ncmm musician`)**
   - 支持细分命令独立调用，方便设置不同的运行频次（如每日运行签到，每月运行进阶任务），规避风控：
     - **身份缓存风控前置**：音乐人身份状态写入本地缓存，缓存命中非音乐人时直接跳过音乐人任务；缓存数据库不可用时不继续请求音乐人接口，避免普通账号误触发音乐人任务链路。
     - **日常签到与云豆任务 (`ncmm musician-sign`)**：每日运行，基于移动端 EAPI 完成音乐人身份确认、音乐人中心签到、周期/阶段任务列表刷新，并自动申领可领取的云豆奖励。
     - **云豆领奖去重缓存**：音乐人云豆领奖本地去重缓存，拦截重复领奖请求。
     - **执行前后任务列表对比**：`musician-sign` 会先显示执行前任务列表，再签到与领奖，最后显示执行后任务列表，任务状态统一展示为“已完成 / 未完成”。
...
- `musician-sign` 已从旧版 WEAPI 音乐人签到实现切换为移动端 EAPI 链路，核心接口包括：
  - `/api/nmusician/workbench/musician/role/get`：确认当前账号是否为音乐人。
  - `/api/creator/user/access`：完成音乐人中心签到，成功依据为 `code=200 && data=true`。
  - `/api/nmusician/workbench/mission/cycle/list`：获取周期任务列表，包含音乐人中心签到任务。
  - `/api/nmusician/workbench/mission/stage/list`：补充阶段任务列表。
  - `/api/nmusician/workbench/mission/reward/obtain/new`：领取云豆奖励，成功依据为 `code=200`。
- EAPI 通用请求字段使用 Cookie 中的 `deviceId/sDeviceId`，默认 `os=iOS`、`verifyId=1`、`e_r=true`，并沿用项目内 EAPI 加解密能力。
- 统一请求层已支持 EAPI 加密响应内部 gzip 解压，并按真实 URL 域名设置 `Host` 请求头，兼容 `interface3.music.163.com` 音乐人接口。
- 云豆领奖成功后会写入本地缓存，缓存 key 按账号 Cookie 文件、`userMissionId` 和 `period` 区分，默认保留 45 天，避免重复领奖调用。
